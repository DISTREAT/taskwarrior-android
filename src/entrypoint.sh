...
toolchain_formatted=$(echo "${ANDROID_TOOLCHAIN}" | tr '[:lower:]' '[:upper:]' | sed 's/-/_/g')
...
-B${ANDROID_NDK_ROOT}/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/${ANDROID_TOOLCHAIN}/${ANDROID_API_LEVEL} \
--sysroot=${ndk_sysroot} \
...
configuration() {
    cmake \
        ...
        -DRust_CARGO_TARGET="${ANDROID_TOOLCHAIN}" \
